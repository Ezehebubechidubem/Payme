...
# register admin blueprint (near other blueprint registrations)
...
def list_staff():
    try:
        conn = get_conn()
        cur = conn.cursor() if DATABASE_URL is None else conn.cursor()
        ...
        for r in rows:
            ...
            staff_list.append({
                ...
                'email': r['email'] if DATABASE_URL else r[2],
                'role': r['role'] if DATABASE_URL else r[3],
            })
